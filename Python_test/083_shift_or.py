...
class ShiftOr:
    ...
    def _build_mask(self, pattern: str) -> Dict[str, int]:
        """构建字符位掩码"""
        m = len(pattern)
        mask = defaultdict(lambda: (1 << m) - 1)  # 默认全1
        
        # 为每个字符构建位掩码
        for i in range(m):
            mask[pattern[i]] &= ~(1 << i)  # 对应位置0
    ...
    def search(self, text: str, pattern: str) -> List[int]:
        ...
        for i, c in enumerate(text):
            # 状态转移：左移一位并与当前字符的位掩码相或
            state = ((state << 1) | mask[c]) & ((1 << m) - 1)
    ...
    def fuzzy_search(self, text: str, pattern: str, k: int) -> List[int]:
        ...
        for i, c in enumerate(text):
            ...
            # 更新每个错误数量的状态
            states[0] = ((old_states[0] << 1) | mask[c]) & ((1 << m) - 1)
            self.steps.append((i, 0, bin(states[0]), "模糊匹配状态"))
            
            for j in range(1, k + 1):
                # 允许j个错误的状态转移
                substitution = ((old_states[j-1] << 1) | mask[c])  # 替换
                deletion = (old_states[j] << 1) | 1                # 删除
                insertion = old_states[j-1] << 1                   # 插入
                
                states[j] = (substitution & deletion & insertion) & ((1 << m) - 1)
                self.steps.append((i, j, bin(states[j]), "错误状态更新"))
            
            # 检查所有状态是否有匹配
            for j in range(k + 1):
                if not (states[j] & (1 << (m - 1))):
    ...
    try:
        ...
        while True:
            ...
            if choice == '1':
                ...
                if pattern:
                    positions = so.search(text, pattern)
                    if positions:
                        print("\n找到的匹配位置：")
                ...
                if pattern:
                    try:
                        ...
                        positions = so.fuzzy_search(text, pattern, k)
                        if positions:
                            print("\n找到的模糊匹配：")
                ...
                pattern = input("请输入要搜索的模式串：").strip()
                if pattern:
                    positions = so.optimize_search(text, pattern)
                    if positions:
                        print("\n找到的匹配位置：")
                        for pos in positions:
                            print(f"位置 {pos}")
